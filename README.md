...
Features Checklist
 Login / Register (Farmer & Worker roles)
 Dashboard with live stats
 Job posting & applications
 Worker profiles & availability toggle (Socket.io)
 Equipment listing & booking
 Nursery & Input marketplace
 AI Crop Recommendation (N,P,K → crop)
 AI Plant Disease Detection (image → disease + treatment)
 AI Soil & Yield Analysis (crop/season/state → yield)
 Government Schemes dashboard
 Market price feed (mock)
 Agriculture calendar
 Community knowledge posts
 Mock payment system
 JWT authentication + role-based access
 MySQL with full schema
